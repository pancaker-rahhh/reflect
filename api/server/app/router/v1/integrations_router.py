...
async def sync_jira_integration(
    ...
    try:
        ...
        result = await jira_integration_service.sync_all_action_items(db, integration)
        ...
        raise HTTPException(
            status_code=500, detail=f'Failed to sync integration: {str(e)}'
        )
...
async def get_sync_status(
    ...
    try:
        ...
        sync_stats = {
            ...
            'synced_items': len([i for i in integrations if i.sync_status == 'synced']),
            'failed_items': len([i for i in integrations if i.sync_status == 'failed']),
            'pending_items': len(
                [i for i in integrations if i.sync_status == 'pending']
            ),
            'last_sync_time': max(
                [i.last_synced_at for i in integrations if i.last_synced_at],
                default=None,
            ),
        }
