...
class IntegrationService:
    async def create_integration(
        ...
        logger.info(f"Created integration {integration.id} for project {integration_data.project_id}")
    ...
    async def get_integration(
        ...
        await organization_service.check_project_access(db, user_id, integration.project_id)
        return IntegrationResponse.model_validate(integration)
    ...
    async def list_integrations(
        ...
        integrations = await integration_repository.get_by_project(db, project_id, skip, limit)
        return [IntegrationResponse.model_validate(integration) for integration in integrations]

    async def update_integration(
        self, db: AsyncSession, user_id: UUID, integration_id: UUID, integration_data: IntegrationUpdate
    ) -> Optional[IntegrationResponse]:
        integration = await integration_repository.get(db, integration_id)
        if not integration:
            return None
        ...
        updated_integration = await integration_repository.update(db, integration_id, **update_dict)
    ...
    async def test_integration(
        ...
        if integration.integration_type == IntegrationType.JIRA:
            ...
            return await jira_service.test_connection(integration.config, integration.auth_data)
            ...
            return await github_service.test_connection(integration.config, integration.auth_data)
        else:
            return {"status": "error", "message": f"Testing not supported for {integration.integration_type}"}

    async def sync_integration(
        self, db: AsyncSession, user_id: UUID, integration_id: UUID, sync_type: str = "incremental"
    ) -> Dict[str, Any]:
        ...
        if not integration.sync_enabled:
            raise HTTPException(status_code=400, detail="Sync is not enabled for this integration")
        
        try:
            await integration_repository.update(db, integration_id, status=IntegrationStatus.ACTIVE)
            ...
            if integration.integration_type == IntegrationType.JIRA:
                ...
                raise HTTPException(status_code=400, detail=f"Sync not supported for {integration.integration_type}")
    ...
    async def create_mapping(
        ...
        mapping = await integration_mapping_repository.create(db, **mapping_data.model_dump())
    ...
    async def list_mappings(
        ...
        mappings = await integration_mapping_repository.get_by_integration(db, integration_id)
        return [IntegrationMappingResponse.model_validate(mapping) for mapping in mappings]
    ...
    async def create_webhook(
        ...
        logger.info(f"Created webhook {webhook.id} for project {webhook_data.project_id}")
    ...
    async def update_webhook(
        self, db: AsyncSession, user_id: UUID, webhook_id: UUID, webhook_data: WebhookUpdate
