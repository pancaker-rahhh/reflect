...
class InvitationService:
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                await self._update_task_status(
                    task_id=task_id, status='completed', db=db, results=results
                )

                logger.info(
                    f'Completed bulk invitation task {task_id}: {success_count} sent, {failed_count} failed'
                )

            except Exception as e:
                logger.error(
                    f'Error processing bulk invitations for task {task_id}: {str(e)}'
                )
                await self._update_task_status(
                    task_id=task_id, status='failed', db=db, error=str(e)
                )
        ...
        try:
            ...
            if existing_invitation:
                existing_invitation.expires_at = datetime.utcnow() + timedelta(
                    hours=self.invitation_expiry_hours
                )
                ...
                invitation = Invitation(
                    ...
                    + timedelta(hours=self.invitation_expiry_hours),
                    created_at=datetime.utcnow(),
                )
                db.add(invitation)
            ...
            email_for_log = (
                invitation_entry.email if invitation_entry.email else 'unknown'
            )
            logger.error(f'Failed to process invitation for {email_for_log}: {str(e)}')
            return InvitationResult(email=email_for_log, status='failed', error=str(e))
    ...
    async def get_invitation_task_status(
        self, task_id: str, user_id: UUID, db: AsyncSession
    ) -> Optional[InvitationStatusResponse]:
        stmt = select(InvitationTask).where(
            and_(InvitationTask.id == task_id, InvitationTask.user_id == user_id)
        )
        result = await db.execute(stmt)
        task = result.scalar_one_or_none()
    ...
    async def get_invitation(
        self, invitation_id: UUID, user_id: UUID, db: AsyncSession
    ) -> Optional[InvitationModel]:
        stmt = select(Invitation).where(
            and_(Invitation.id == invitation_id, Invitation.invited_by == user_id)
        )
        result = await db.execute(stmt)
    ...
    async def validate_invitation_token(
        ...
        stmt = (
            ...
            .where(and_(Invitation.token == token, Invitation.status == 'pending'))
        )
        result = await db.execute(stmt)
        invitation = result.scalar_one_or_none()
    ...
    async def accept_invitation(
        ...
        if bool(validation.is_expired):  # type: ignore
        ...
        if invitation.organization_id:
            redirect_url = (
                f'{settings.FRONTEND_URL}/org/{invitation.organization_id}/dashboard'
            )
    ...
    async def _create_user_from_invitation(
        self, invitation: Invitation, user_data: NewUserData, db: AsyncSession
    ) -> User:
    ...
    async def _add_to_organization(
        self, user_id: UUID, organization_id: UUID, role: str, db: AsyncSession
    ) -> None:
        stmt = select(OrganizationMember).where(
            and_(
                OrganizationMember.organization_id == organization_id,
