...
class OnboardingService:
    ...
    async def get_onboarding_status(
        ...
        try:
            stmt = (
                ...
                .options(joinedload(User.onboarding))
            )
            ...
            return OnboardingStatusResponse(
                ...
                has_created_project=onboarding.has_created_project
                if onboarding
                else False,
            ...
            logger.error(
                f'Failed to get onboarding status for user {user_id}: {str(e)}'
            )
            raise
    ...
    async def update_onboarding(
        ...
        try:
            ...
            if not user.onboarding:
                ...
                await (
                    db.flush()
                )  # Ensure the onboarding record is persisted before accessing it
            ...
            onboarding_has_created_project = (
                user.onboarding.has_created_project if user.onboarding else False
            )
            onboarding_created_at = (
                user.onboarding.created_at if user.onboarding else None
            )
            onboarding_updated_at = (
                user.onboarding.updated_at if user.onboarding else None
            )
            ...
            logger.error(
                f'Failed to update onboarding for user {user_id}: {str(e)}',
                exc_info=True,
            )
            await db.rollback()
            raise
    ...
    async def check_first_time_user(self, user_id: uuid.UUID, db: AsyncSession) -> bool:
        try:
            user = await db.get(User, user_id)
            if not user:
                return True
            ...
            metadata = user.user_metadata or {}
            if metadata.get('onboarding_completed') or metadata.get(
                'onboarding_skipped'
            ):
                return False
            ...
            logger.error(
                f'Failed to check first-time user status for {user_id}: {str(e)}'
            )
            return True
