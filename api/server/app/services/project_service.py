...
from app.schemas.project import ProjectCreate, ProjectUpdate, ProjectSettings, ProjectSettingsUpdate
...
async def _check_workspace_access(db: AsyncSession, user: User, workspace_id: UUID):
    ...
    logging.warning(f"Workspace access check for user {user.id} and workspace {workspace_id} is currently a placeholder.")
    return True
...
class ProjectService:
    ...
    async def create_project(self, db: AsyncSession, user: User, project_in: ProjectCreate) -> Project:
        ...
        suffix = 1
        while await self.repository.get_by_workspace_and_slug(db, project_in.workspace_id, unique_slug):
        ...
        project_obj = Project(**project_data, slug=unique_slug)
    ...
    async def update_project_settings(
        self, db: AsyncSession, user: User, project_id: UUID, settings_in: ProjectSettingsUpdate
    ) -> dict:
        project = await self.get_project_and_check_access(db, user, project_id)
        
        update_data = settings_in.model_dump(exclude_unset=True)
        current_settings = project.settings.copy() if isinstance(project.settings, dict) else {}
        current_settings.update(update_data)
        validated_settings = ProjectSettings.model_validate(current_settings).model_dump()
        await self.repository.update(db, id=project_id, settings=validated_settings)
        
        return validated_settings

    async def delete_project(self, db: AsyncSession, user: User, project_id: UUID) -> Project:
        project_to_delete = await self.get_project_and_check_access(db, user, project_id)
