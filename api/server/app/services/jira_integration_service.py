...
class JIRAIntegrationService:
    ...
    async def test_connection(self, config: Dict[str, Any], auth_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            async with session.get(f"{base_url}/rest/api/3/myself", headers=headers) as response:
    ...
    async def get_project_info(self, config: Dict[str, Any], auth_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            base_url = config.get('base_url')
            project_key = config.get('project_key')
            
            if not base_url or not project_key:
                return {"status": "error", "message": "JIRA base URL and project key are required"}
            
            headers = self._get_auth_header(auth_data)
            session = await self.get_session()
            
            async with session.get(f"{base_url}/rest/api/3/project/{project_key}", headers=headers) as response:
                if response.status == 200:
                    project_info = await response.json()
                    return {"status": "success", "project": project_info}
                else:
                    error_text = await response.text()
                    return {"status": "error", "message": f"Failed to get project info: {error_text}"}
    ...
    async def create_issue(
        self, config: Dict[str, Any], auth_data: Dict[str, Any], issue_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        try:
            base_url = config.get('base_url')
            project_key = config.get('project_key')
            ...
            async with session.post(
                ...
                if response.status == 201:
                    ...
                    return {"status": "error", "message": f"Failed to create issue: {error_text}"}
    ...
    async def update_issue(
        self, config: Dict[str, Any], auth_data: Dict[str, Any], issue_key: str, update_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        try:
            base_url = config.get('base_url')
            headers = self._get_auth_header(auth_data)
            ...
            async with session.put(
                ...
                if response.status == 204:
                    ...
                    error_text = await response.text()
                    return {"status": "error", "message": f"Failed to update issue: {error_text}"}
    ...
    async def export_feedback_to_jira(
        ...
        try:
            ...
            if feedback.priority is not None:
                ...
                issue_data["priority"] = priority_mapping.get(feedback.priority.value.lower(), "Medium")
            
            result = await self.create_issue(integration.config, integration.auth_data, issue_data)
            ...
            if result.get("status") == "success":
                ...
                logger.info(f"Exported feedback {feedback_id} to JIRA issue {result.get('key')}")
    ...
    async def sync_data(
        ...
        try:
            ...
            if sync_type == "full":
                mappings = await integration_mapping_repository.get_by_integration(db, integration.id)
                
                for mapping in mappings:
                    try:
                        if mapping.mapping_type == MappingType.FEEDBACK_TO_ISSUE:
                            feedback = await feedback_repository.get(db, mapping.internal_id)
                            if feedback:
                                ...
                                if result.get("status") == "success":
                                    ...
                                    errors.append(f"Failed to sync {mapping.external_id}: {result.get('message')}")
