...
class GitHubIntegrationService:
    ...
    async def get_repository_info(self, config: Dict[str, Any], auth_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            repo_owner = config.get('repo_owner')
            repo_name = config.get('repo_name')
            
            if not repo_owner or not repo_name:
                return {"status": "error", "message": "GitHub repo owner and name are required"}
            
            headers = self._get_auth_header(auth_data)
            session = await self.get_session()
            
            async with session.get(f"{self.base_url}/repos/{repo_owner}/{repo_name}", headers=headers) as response:
                if response.status == 200:
                    repo_info = await response.json()
                    return {"status": "success", "repository": repo_info}
                else:
                    error_text = await response.text()
                    return {"status": "error", "message": f"Failed to get repository info: {error_text}"}
    ...
    async def update_issue(
        self, config: Dict[str, Any], auth_data: Dict[str, Any], issue_number: str, update_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        try:
            repo_owner = config.get('repo_owner')
            repo_name = config.get('repo_name')
    ...
    async def export_feedback_to_github(
        ...
        try:
            ...
            if result.get("status") == "success":
                ...
                logger.info(f"Exported feedback {feedback_id} to GitHub issue #{result.get('number')}")
                        ...
                        if mapping.mapping_type == MappingType.FEEDBACK_TO_ISSUE:
                            ...
                            if feedback:
                                ...
                                if result.get("status") == "success":
                                    ...
                                    errors.append(f"Failed to sync issue #{mapping.external_id}: {result.get('message')}")
