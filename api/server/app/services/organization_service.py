...
class OrganizationService:
    ...
    async def get_organization(
        self, org_id: UUID, user_id: UUID, db: AsyncSession, include_members: bool = False
    ) -> OrganizationResponse:
    ...
    async def update_organization(
        ...
        try:
            updated_org = await organization_repository.update(db, org_id, **update_dict)
            if not updated_org:
                raise NotFoundError("Organization not found")
    ...
    async def get_user_membership(
        ...
        member = await organization_member_repository.get_by_org_and_user(db, org_id, user_id)
        if not member:
            return None
    ...
    async def leave_organization(
        ...
        if member.role == OrganizationRole.OWNER:
            member_count = await organization_member_repository.count_org_members(db, org_id)
            if member_count > 1:
                raise ForbiddenError(
                    "Organization owner cannot leave. Transfer ownership first or delete the organization."
                )
        
        success = await organization_member_repository.remove_member(db, org_id, user_id)
        if success:
