...
const generateFeedback = (): Feedback[] => {
  ...
  // Generate reviews
  const reviewTitles = ['Excellent Service', 'Could be better', 'Amazing product!', 'Satisfied customer']
  for (let i = 0; i < 8; i++) {
    const review: Review = {
      id: `review-${i}`,
      projectId: 'project-1',
  ...
  const bugTitles = ['Login button not working', 'Page loads slowly', 'Error on checkout', 'Mobile layout issue']
  for (let i = 0; i < 5; i++) {
    const bug: BugReport = {
      id: `bug-${i}`,
      projectId: 'project-1',
      ...
      description: 'When I try to click the button, nothing happens. This started happening after the last update.',
  ...
  const featureTitles = ['Dark mode support', 'Export to PDF', 'Team collaboration', 'API access', 'Mobile app']
  for (let i = 0; i < 7; i++) {
    const feature: FeatureRequest = {
      ...
      description: 'It would be great if we could have this feature. It would really help our team be more productive.',
      status: (['new', 'under-review', 'planned', 'in-progress'] as const)[Math.floor(Math.random() * 4)],
