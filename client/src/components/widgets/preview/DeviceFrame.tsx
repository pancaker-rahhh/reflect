    ...
    switch (deviceType) {
      ...
      default:
        return {
          ...
          frame: 'w-full h-full bg-muted rounded-lg p-4 shadow-2xl',
          screen:
            'w-full h-full bg-background rounded overflow-hidden relative border border-border',
          notch: false,
        }
    }
  }
  ...
  return (
    <div className={frameStyles.container}>
      <div className={frameStyles.frame}>
        {/* Screen */}
        <div className={frameStyles.screen}>{children}</div>
      ...
      {/* Device Label */}
      <div className="text-center mt-2 text-xs text-muted-foreground capitalize">
        {deviceType} Preview
      </div>
    </div>
  )
